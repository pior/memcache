...
Source material:
- The Meta protocol is documented in references/meta-protocol.txt, use it as a primary source of truth.
...
The memcache-bench tool:
- Implement different type of operations:
    - cache-hit: 1 set then 100 get
    - dynamic-value: 1 set then 1 get
    - cache-miss: 1 get (on inexistant key)
    - increment: 100 incr then 1 get (to check the value)
    - delete: 1 set then 1 delete
- Verify the correctness and measure the performance of each operation.
- The cli should allow to run a specific operation, or all operations in a loop, for a configurable duration (default: 5 seconds).
- The CLI should allow to specify the concurrency level (default: 1)
