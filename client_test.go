...
func newTestClient(t *testing.T, maxIdleConns int) Client {
	...
	client, err := NewClient(ServersFromAddr(testMemcachedHost), config)
	if err != nil {
...
func TestClient_MetaSetGetDelete(t *testing.T) {
...
func TestClient_MetaArithmetic(t *testing.T) {
...
func TestClient_MetaNoop(t *testing.T) {
	client := newTestClient(t, 2)
	defer client.Close()

	resp, err := client.MetaNoop("key")
	if err != nil {
...
func TestClient_MetaGetMiss(t *testing.T) {
	client := newTestClient(t, 2)
	defer client.Close()
...
func TestClient_CustomDialFunc(t *testing.T) {
	var dialerUsed bool
...
func TestClient_DialTimeout(t *testing.T) {
	...
	client, err := NewClient(ServersFromAddr(nonRoutableAddress), config)
	if err != nil {
	...
	startTime := time.Now()
	_, err = client.MetaNoop("key") // This should trigger a dial and timeout
...
func TestClient_IdleTimeout(t *testing.T) {
	...
	c, err := NewClient(ServersFromAddr(testMemcachedHost), config)
	if err != nil {
		t.Fatalf("Failed to create client: %v", err)
	}
	defer c.Close()
	...
	_, err = c.MetaNoop("key")
	if err != nil {
		t.Fatalf("MetaNoop failed: %v", err)
	}

	// Check that there is one connection in the pool
	count := c.getPool("key").connectionsCount()
	if count != 1 {
		t.Fatalf("Expected 1 free connection, got %d", count)
	}
	...
	count = c.getPool("key").connectionsCount()
	if count != 1 {
		t.Logf("Expected 1 free connection after idle timeout and new op, got %d. This might be acceptable if the old conn was closed.", count)
	}

	// Test that a connection used just before timeout is not closed
	_, err = c.MetaNoop("key") // Use conn1, goes to pool
	if err != nil {
		t.Fatalf("MetaNoop failed: %v", err)
	}
	time.Sleep(idleTimeout / 2) // Wait half the timeout

	_, err = c.MetaNoop("key") // Use conn2, goes to pool (conn1 is now older)
	if err != nil {
		t.Fatalf("MetaNoop failed: %v", err)
	}
	...
	count = c.getPool("key").connectionsCount()
	if count != 1 {
		t.Errorf("Expected 1 connection in pool after staggered use, found %d", count)
	}
}
...
// func TestClient_CondRelease(t *testing.T) {
