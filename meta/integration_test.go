...
func TestIntegration_GetWithFlags(t *testing.T) {
	...
	getReq := NewRequest(CmdGet, "test_flags_key", nil,
		Flag{Type: FlagReturnValue},       // v - return value data
		Flag{Type: FlagReturnCAS},         // c - return CAS token
		Flag{Type: FlagReturnTTL},         // t - return remaining TTL
...
func TestIntegration_Pipelining(t *testing.T) {
	...
	reqs := []*Request{
		NewRequest(CmdGet, "pipe_key1", nil,
			Flag{Type: FlagReturnValue}, // v - return value
			Flag{Type: FlagReturnKey},   // k - return key in response
			Flag{Type: FlagQuiet},       // q - suppress miss response
		),
		NewRequest(CmdGet, "pipe_key2", nil,
			Flag{Type: FlagReturnValue},
			Flag{Type: FlagReturnKey},
			Flag{Type: FlagQuiet},
		),
		NewRequest(CmdGet, "pipe_key3", nil,
			Flag{Type: FlagReturnValue},
			Flag{Type: FlagReturnKey},
			Flag{Type: FlagQuiet},
		),
		NewRequest(CmdGet, "nonexistent", nil, // This won't return due to quiet mode
			Flag{Type: FlagReturnValue},
			Flag{Type: FlagReturnKey},
			Flag{Type: FlagQuiet},
		),
